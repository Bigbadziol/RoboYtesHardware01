...
const char* _spp_server_name_2 = "ESP32SPP_PIN";
...
static void esp_spp_cb(esp_spp_cb_event_t event, esp_spp_cb_param_t* param)
{
    ...
    case ESP_SPP_INIT_EVT:
        ...
        if (!_isMaster) {
            ...
            esp_spp_start_srv(ESP_SPP_SEC_NONE, ESP_SPP_ROLE_SLAVE, 0, _spp_server_name_2);
        }
...
void BdlBluetoothSerial::onData(BluetoothSerialDataCb cb) {
...
BdlBluetoothSerial::BdlBluetoothSerial()
{
    local_name = "ESP32"; //default bluetooth name
}

BdlBluetoothSerial::~BdlBluetoothSerial(void)
{
    _stop_bt();
}

bool BdlBluetoothSerial::begin(String localName, bool isMaster)
{
    _isMaster = isMaster;
    if (localName.length()) {
...
int BdlBluetoothSerial::available(void)
{
    if (_spp_rx_queue == NULL) {
        return 0;
    }
...
int BdlBluetoothSerial::peek(void)
{
    uint8_t c;
...
bool BdlBluetoothSerial::hasClient(void)
{
    return _spp_client > 0;
}

int BdlBluetoothSerial::read(void)
{
...
size_t BdlBluetoothSerial::write(uint8_t c)
{
    return write(&c, 1);
}

size_t BdlBluetoothSerial::write(const uint8_t* buffer, size_t size)
{
    if (!_spp_client) {
        return 0;
    }
...
void BdlBluetoothSerial::end()
{
    _stop_bt();
}

void BdlBluetoothSerial::onConfirmRequest(ConfirmRequestCb cb)
{
    confirm_request_callback = cb;
}

void BdlBluetoothSerial::onAuthComplete(AuthCompleteCb cb)
{
    auth_complete_callback = cb;
}

void BdlBluetoothSerial::confirmReply(boolean confirm)
{
    esp_bt_gap_ssp_confirm_reply(current_bd_addr, confirm);
}


esp_err_t BdlBluetoothSerial::register_callback(esp_spp_cb_t* callback)
{
...
void BdlBluetoothSerial::enableSSP() {
    _enableSSP = true;
}
/*
     ...
     * Use fixed pin code
*/
bool BdlBluetoothSerial::setPin(const char* pin) {
...
bool BdlBluetoothSerial::connect(String remoteName)
{
...
bool BdlBluetoothSerial::connect(uint8_t remoteAddress[])
{
    if (!isReady(true, READY_TIMEOUT)) return false;
    if (!remoteAddress) {
...
bool BdlBluetoothSerial::connect()
{
    if (!isReady(true, READY_TIMEOUT)) return false;
    if (_isRemoteAddressSet) {
        disconnect();
...
bool BdlBluetoothSerial::disconnect() {
    if (_spp_client) {
        flush();
        log_i("disconnecting");
...
bool BdlBluetoothSerial::unpairDevice(uint8_t remoteAddress[]) {
    if (isReady(false, READY_TIMEOUT)) {
...
bool BdlBluetoothSerial::connected(int timeout) {
    return waitForConnect(timeout);
}

bool BdlBluetoothSerial::isReady(bool checkMaster, int timeout) {
 ...
 */
BTScanResults* BdlBluetoothSerial::discover(int timeoutMs) {
    scanResults.clear();
 ...
 */
bool BdlBluetoothSerial::discoverAsync(BTAdvertisedDeviceCb cb, int timeoutMs) {
    scanResults.clear();
...
void BdlBluetoothSerial::discoverAsyncStop() {
    esp_bt_gap_cancel_discovery();
    advertisedDeviceCb = nullptr;
}

/** @brief      Clears scanresult entries */
void BdlBluetoothSerial::discoverClear() {
    scanResults.clear();
}
 ...
 */
BTScanResults* BdlBluetoothSerial::getScanResults() {
    return &scanResults;
}

BdlBluetoothSerial::operator bool() const
{
    return true;
}
#endif
